...
"""
This is version nr 2
Added:
- dropout and batch normalization
- enlarge model layers and embedding dimensions
- add lstm layer and dense
"""
...
def create_model():
    ...
    city_embedding = keras.layers.Embedding(N_cities+1, 36, trainable=True)(input_city_seq)

    lstm = keras.layers.Bidirectional(keras.layers.LSTM(256, return_sequences=True, dropout=0.15))(city_embedding)
    lstm = keras.layers.Bidirectional(keras.layers.LSTM(256, return_sequences=True, dropout=0.15))(lstm)
    gru = keras.layers.Bidirectional(keras.layers.GRU(256, return_sequences=False, dropout=0.15))(lstm)
    ...
    batch_normalized_categorical = keras.layers.BatchNormalization()(embedding_categorical)
    dense_categorical = keras.layers.Dense(512, activation='relu')(batch_normalized_categorical)
    ...
    batch_normalized_continuous = keras.layers.BatchNormalization()(input_continuous)

    # Final MLP
    all_together = keras.layers.concatenate([gru, flatten_categorical, batch_normalized_continuous], axis=1)
